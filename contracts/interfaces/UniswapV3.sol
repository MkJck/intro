...
library TickMath {
    ...
    function getSqrtRatioByPrice(uint256 price, uint256 decimals) internal pure returns (uint160) {
        return SafeCast.toUint160(sqrt(FullMath.mulDiv(price, 2 ** 192, 10 ** decimals)));
    }

    function getPriceBySqrtRatio(uint160 sqrtRatio, uint256 decimals) internal pure returns (uint256) {
        return FullMath.mulDiv(uint256(sqrtRatio), uint256(sqrtRatio) * 10 ** decimals, 2 ** 192);
    }

    function priceToTick(uint256 price, uint256 dec0, int24 tickSpacing) internal pure returns (int24 tick) {

        tick = getTickAtSqrtRatio(getSqrtRatioByPrice(price, dec0));

        if (tick % tickSpacing != 0) {
            tick = tick > 0 ? tick - tick % tickSpacing : tick - tickSpacing - (tick % tickSpacing);
        }
    }
...
library SafeCast {
    ...
    function x96toUint256(uint160 x) internal pure returns (uint256 y) {
        y = uint256(x / (2 ** 96));
    }
}
